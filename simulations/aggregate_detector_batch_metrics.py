...
def main():
    ...
    for dir in os.listdir(ROOT):
        if dir.startswith("2024"):
            for input_sensor_file in os.listdir(ROOT + dir):
                if input_sensor_file.startswith("drakewell"):
                    print(f"Processing {input_sensor_file} in {dir}")
                    input_sensor_file = os.path.join(ROOT, dir, input_sensor_file)
                    output_file = "test.out"
                    demand_file = os.path.join(ROOT, dir, "demand_data.xml")
                    ...
                    MSE.append(mse)
                    MAPE.append(mape)
    average_mae = np.nanmean(MAE)
    average_mse = np.nanmean(MSE)
